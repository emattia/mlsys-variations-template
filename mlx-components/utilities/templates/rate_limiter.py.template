...
class RateLimiter:
    ...
    def _check_limits(self, service: str, cost: float) -> bool:
        ...
        if cost_info["daily"] + cost > limits.cost_limit_per_day:
            logger.warning(f"Cost limit exceeded for {service}: ${cost_info['daily']:.2f}/day")
            return False
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    future = asyncio.ensure_future(limiter.acquire(service, cost))
                    # This is a limitation - sync functions in async contexts need special handling
                    allowed = True  # Fallback to allowing the request
                else:
...
def estimate_openai_cost(model: str, input_tokens: int, output_tokens: int = 0) -> float:
