...
class CacheManager:
    ...
    def __init__(self, cache_dir: Path | str = DEFAULT_CACHE_DIR):
        ...
        # Get max memory items from config, with fallback to default
        try:
            from hydra import compose
            from omegaconf import OmegaConf

            # Try to get from config, fall back to default if not available
            try:
                if hasattr(OmegaConf, "structured"):
                    cfg = compose(config_name="config")
                    self.max_memory_items = (
                        cfg.get("ml_systems", {})
                        .get("caching", {})
                        .get("memory", {})
                        .get("max_memory_items", 1000)
                    )
                else:
