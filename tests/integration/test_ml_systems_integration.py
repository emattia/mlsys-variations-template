...
from src.utils.rate_limiter import RateLimit, RateLimiter, rate_limited
from src.utils.cache_manager import CacheManager, cached_llm_call
from src.utils.templates import TemplateManager, render_prompt
...
class TestMLSystemsIntegration:
    ...
    async def test_llm_pipeline_with_all_components(self, integrated_components):
        ...
        test_inputs = [
            ...
            {"text": "This is terrible quality", "categories": "positive, negative, neutral"},
            {"text": "I love this product!", "categories": "positive, negative, neutral"},  # Duplicate for cache test
        ]
        ...
        for i, input_data in enumerate(test_inputs):
            ...
            rate_allowed = await components["rate_limiter"].acquire("test_llm", cost=0.02)
            
            if rate_allowed:
                try:
                    # 3. Check cache first
                    cached_response = components["cache_manager"].get_llm_response(prompt, "gpt-4")
        ...
        assert len(successful_results) >= 2, f"Expected at least 2 successful results, got {len(successful_results)}"
        ...
        assert request_count >= 2 and request_count <= 3, f"Expected 2-3 requests, got {request_count}"
    ...
    def test_ab_testing_with_cost_optimization(self, integrated_components):
        ...
        for user_id in range(20):
            ...
            performance_score = 0.8 + (0.1 if assigned_version == "v2" else 0)  # v2 performs better
        ...
        assert ab_results["avg_metrics_b"]["performance"] > ab_results["avg_metrics_a"]["performance"]  # v2 should perform better
        
        # Check cost optimization
        cache_stats = components["cache_manager"].get_stats()
        assert float(cache_stats["total_cost_saved"].replace("$", "")) > 0  # Some cost should be saved
        
        # Check rate limiting didn't block requests
        rate_status = components["rate_limiter"].get_status("test_llm")
    ...
    async def test_concurrent_usage_with_all_systems(self, integrated_components):
        ...
        async def simulate_user_session(user_id: int, session_requests: int = 3):  # Reduced from 5 to 3
            ...
            for req_id in range(session_requests):
                try:
                    ...
                    if await components["rate_limiter"].acquire("test_llm", cost=0.02):
                        # Check cache
                        cached = components["cache_manager"].get_llm_response(prompt, "gpt-4")
        ...
        rate_limited_requests = sum(1 for r in flat_results if not r["success"] and "Rate limited" in r.get("error", ""))
        
        # With reduced load, most requests should succeed (within rate limits of 10/min)
        success_rate = successful_requests / total_requests
        assert success_rate >= 0.7, f"Success rate too low: {success_rate:.2%} (expected >= 70%)"
        ...
        has_caching_evidence = (
            cached_requests > 0 or 
            cache_stats["total_entries"] < total_requests  # Less entries than requests means some were cached
        )
        ...
        assert total_cost <= max_possible_cost, f"Cost should not exceed maximum: {total_cost} vs {max_possible_cost}"
    ...
    def test_template_versioning_with_performance_tracking(self, integrated_components):
        ...
        for scenario in test_scenarios:
            ...
            for i in range(scenario["usage_count"]):
                ...
                template_data = components["template_manager"].templates["prompts"][version]["classification_prompt"]
        ...
        # Get analytics
        analytics = components["template_manager"].get_template_analytics("classification_prompt")
    ...
    def test_error_handling_and_recovery(self, integrated_components):
        ...
        test_cases = [
            {
                "name": "Cache failure",
                "setup": lambda: components["cache_manager"].cache_dir.chmod(0o000),  # Make read-only
            ...
            },
            {
                ...
                "setup": lambda: components["rate_limiter"]._check_limits("test_llm", 1000.0),  # Exceed cost limit
                    ...
                    if prompt is None:
                        ...
                        try:
                            ...
                            results.append({
                                "test": test_case["name"],
                                "result": "success",
                                "expected": "Cache failure" not in test_case["name"]
                            })
        ...
        for result in results:
            if "Template not found" in result["test"]:
                assert result["result"] == "template_failed", f"Template error should be handled: {result}"
            
            # System should remain stable even with failures
            assert "error" not in result or isinstance(result["error"], str), "Errors should be properly formatted"
...
class TestProductionScenarios:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            templates = [
                ...
                },
                {
                    ...
                    "template": "Content: {content}\nContext: {context}\n\nIs this content appropriate? (Yes/No)\nReason:",
                    "version": "v1",
    ...
    def test_high_throughput_scenario(self, production_setup):
        ...
        for i in range(total_requests):
            try:
                ...
                if cached_response:
                    ...
                    import asyncio
                    can_proceed = asyncio.run(components["rate_limiter"].acquire("openai", cost=0.02))
        ...
        assert total_cost < total_requests * 0.02 * 0.7, "Cost savings from caching should be significant"
        
        # System health checks
        cache_stats = components["cache_manager"].get_stats()
        rate_status = components["rate_limiter"].get_status("openai")
        
        assert float(cache_stats["hit_rate"].rstrip('%')) > 30, "Cache hit rate should be healthy"
    ...
    def test_cost_budget_enforcement(self, production_setup):
        ...
        for i in range(100):  # Try 100 requests at $0.02 each = $2.00 total
            import asyncio
            can_proceed = asyncio.run(components["rate_limiter"].acquire("openai", cost=0.02))
        ...
        assert requests_made <= 50, f"Too many requests allowed: {requests_made}"  # 50 * $0.02 = $1.00
        
        print(f"Budget enforcement test:")
