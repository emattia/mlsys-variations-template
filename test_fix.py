...
class TestFixedIntegration:
    ...
    async def test_fixed_llm_pipeline(self, fixed_components):
        ...
        assert len(successful_results) == 3, f"Expected all requests to succeed, but only {len(successful_results)} succeeded"
        ...
        assert requests_made >= 2, f"Expected at least 2 requests recorded, got {requests_made}"
        assert requests_made <= 3, f"Expected at most 3 requests recorded, got {requests_made}"
