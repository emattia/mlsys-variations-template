...
class AnalyticsDashboard:
                    ...
                    alerts.append(
                        {
                            ...
                            * 100,
                            "severity": "high"
                            if recent_mean < historical_mean - (decline_threshold * 2)
                            else "medium",
                        }
                    )
    ...
    def display_overview_dashboard(self):
        ...
        if not metrics:
            console.print(
                Panel(
                    "[yellow]No evaluation data found. Run some evaluations first![/yellow]"
                )
            )
            return
        ...
        layout["left"].split_column(
            Layout(name="performance", ratio=1), Layout(name="categories", ratio=1)
        )

        layout["right"].split_column(
            Layout(name="trends", ratio=1), Layout(name="alerts", ratio=1)
        )
    ...
    def _create_trends_panel(self, metrics: dict[str, Any]) -> Panel:
        ...
        if framework_coverage:
            ...
            for framework, count in sorted(
                framework_coverage.items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def _create_alerts_panel(self, metrics: dict[str, Any]) -> Panel:
        ...
        if not alerts:
            loaded_data = "[green]✅ No performance regressions detected![/green]"
        else:
        ...
        return Panel(loaded_data, title="🚨 Regression Alerts", border_style="red")
    ...
    def _display_category_details(self, metrics: dict[str, Any], category: str):
        ...
        table = Table(
            title=f"Performance Breakdown: {category.title()}", show_header=True
        )
        table.add_column("Dimension", style="cyan")
        ...
        recent_evals = [
            e for e in self.evaluations[-10:] if e.get("query_category") == category
        ]
        if recent_evals:
            ...
            for i, eval_data in enumerate(recent_evals[-5:], 1):
                ...
                grade = eval_data.get("grade", "F")
                query = (
                    eval_data.get("user_query", "")[:60] + "..."
                    if len(eval_data.get("user_query", "")) > 60
                    else eval_data.get("user_query", "")
                )
...
def export_report(
    output: str = typer.Option(
        "reports/mlx_ai_metrics_report.json", "--output", "-o", help="Output file path"
    ),
):
    """Export comprehensive metrics report"""
    dashboard = AnalyticsDashboard()
