...
class NamingMigrator:
    ...
    def generate_updated_files(self, config: NamingConfig):
        ...
        if old_cli_path.exists() and old_cli_path != new_cli_path:
            console.print(
                f"📝 CLI file should be renamed: {old_cli_path} → {new_cli_path}"
            )
        ...
        if setup_path.exists():
            ...
            if loaded_data != loaded_data:
                console.print("📝 setup.py needs updates for CLI naming")
...
def main():
    ...
    def analyze_naming():
        ...
        console.print(
            "[yellow]📋 Found naming patterns that could be migrated:[/yellow]\n"
        )
    ...
    def migrate_files(
        ...
        console.print(
            f"🔄 [yellow]{'Analyzing' if dry_run else 'Migrating'} evaluation files...[/yellow]\n"
        )
