...
def run_test_suite():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            console.print(
                f"❌ [red]Platform CLI functionality: FAILED - {result.stderr}[/red]"
            )
        ...
        if result.returncode == 0:
            ...
            console.print(
                f"❌ [red]Evaluation CLI functionality: FAILED - {result.stderr}[/red]"
            )
    ...
    try:
        result = subprocess.run(
            [
                sys.executable,
                "scripts/migrate_platform_naming.py",
                "migrate",
                "--dry-run",
            ],
            capture_output=True,
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            console.print(
                f"❌ [red]Naming pattern analysis: FAILED - {result.stderr}[/red]"
            )
    ...
    console.print("\n" + "=" * 60)
    console.print(
        Panel.fit("[bold cyan]🎯 Test Results Summary[/bold cyan]", title="Results")
    )
    ...
    for test_name, status, error in test_results:
        if status == "PASSED":
            ...
            failed_count += 1
        error_display = (
            error[:37] + "..." if error and len(error) > 40 else (error or "")
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "Found naming patterns" in result.stdout:
                ...
                console.print(
                    "💡 [dim]Consider running a migration to fix these issues:[/dim]"
                )
                console.print(
                    "   [cyan]python scripts/migrate_platform_naming.py set-preset mlx --apply[/cyan]"
                )
                console.print(
                    "   [cyan]python scripts/migrate_platform_naming.py set-preset mlsys --apply[/cyan]"
                )
                return False
            else:
