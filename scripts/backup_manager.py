...
class BackupManager:
        ...
        report = {
            ...
            "categories": {
                "active": {
                    ...
                    "size_mb": round(
                        sum(b.size_bytes for b in categories["active"]) / (1024 * 1024),
                        2,
                    ),
                ...
                "archivable": {
                    ...
                    "size_mb": round(
                        sum(b.size_bytes for b in categories["archivable"])
                        / (1024 * 1024),
                        2,
                    ),
                ...
                "expired": {
                    ...
                    "size_mb": round(
                        sum(b.size_bytes for b in categories["expired"])
                        / (1024 * 1024),
                        2,
                    ),
    ...
    def _generate_recommendations(self, categories: dict) -> list[str]:
        ...
        orphaned = [
            b for cat in categories.values() for b in cat if not b.source_exists
        ]
        if orphaned:
            recommendations.append(
        ...
        if not recommendations:
            recommendations.append(
                "EXCELLENT: No cleanup needed - backup management is optimal"
            )
    ...
    def cleanup_backups(self, dry_run: bool = True) -> dict:
        ...
        if dry_run:
            ...
            logger.info(
                f"Cleanup completed - freed {cleanup_stats['total_freed_mb']} MB"
            )
...
def main():
    ...
    parser.add_argument(
        "--output", default="backup_audit_report.json", help="Output file for reports"
    )
