...
class DeadCodeAnalyzer:
    ...
    def analyze_file(self, file_path: Path) -> Optional[ASTAnalyzer]:
        ...
        try:
            ...
            logger.info(f"Analyzed {file_path}: {len(analyzer.defined_functions)} functions, "
    ...
    def identify_dead_code(self) -> List[DeadCodeItem]:
        ...
        for func_key, func_info in all_defined_functions.items():
            ...
            is_called = (func_name in all_function_calls or 
                        any(func_name in call for call in all_function_calls))
        ...
        for import_key, import_info in all_imports.items():
            ...
            is_used = import_name in all_variable_uses or import_name in all_function_calls
    ...
    def generate_report(self, output_file: str = "dead_code_report.json") -> Dict[str, Any]:
        ...
        report = {
            "analysis_timestamp": "2025-06-17T22:51:44",
            "statistics": stats,
    ...
    def _generate_recommendations(self, categories: Dict, stats: Dict) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("EXCELLENT: Minimal dead code detected - code quality is high")
    ...
    if args.scan_all:
        ...
        if high_confidence_items:
            ...
            for item in high_confidence_items[:10]:  # Show top 10
                print(f"  • {item.file_path}:{item.line_number} - {item.item_type} '{item.name}' "
