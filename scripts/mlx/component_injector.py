...
class ComponentInjector:
    ...
    def _check_dependencies(self, component_meta: Dict, force: bool) -> bool:
        ...
        installed_components = self.project_config.get("platform", {}).get("components", [])
    ...
    def _inject_component_files(self, component_name: str, component_meta: Dict) -> bool:
        ...
        for template_file in template_dir.rglob("*.template"):
            ...
            final_content = self._substitute_template_variables(template_content, template_vars)
            
            # Determine merge strategy
            merge_strategy = self._get_merge_strategy(str(target_path), component_meta)
            
            # Apply merge strategy
            if not self._apply_merge_strategy(target_path, final_content, merge_strategy):
    ...
    def _get_template_variables(self, component_meta: Dict) -> Dict[str, str]:
        ...
        if "platform" in self.project_config and "name" in self.project_config["platform"]:
    ...
    def _determine_target_path(self, template_file: Path, component_meta: Dict) -> Optional[Path]:
    ...
    def _substitute_template_variables(self, content: str, variables: Dict[str, str]) -> str:
    ...
    def _apply_merge_strategy(self, target_path: Path, content: str, strategy: str) -> bool:
    ...
    def _merge_yaml_files(self, target_path: Path, new_content: str) -> bool:
        ...
        try:
            ...
            print(f"⚠️  Could not merge YAML files: {e}")
            return False
    ...
    def _enhance_file(self, target_path: Path, content: str) -> bool:
        """Enhance existing file with new content."""
        try:
            ...
            if "# MLX_INJECT_POINT" in existing_content:
                enhanced_content = existing_content.replace("# MLX_INJECT_POINT", content)
            else:
                # Append to end
                enhanced_content = existing_content + f"\n\n# Enhanced by MLX\n{content}\n"
