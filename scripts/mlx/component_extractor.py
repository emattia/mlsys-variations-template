        ...
        self.classes.append(
            {
                ...
                "decorators": [
                    self._extract_decorator_name(dec) for dec in node.decorator_list
                ],
            }
        )
        self.generic_visit(node)
            ...
            if hasattr(node.func.value, "id"):
                if node.func.value.id in ["requests", "httpx", "aiohttp"]:
                    self.external_calls.append(
                        {"library": node.func.value.id, "method": node.func.attr}
                    )
...
class ProductionComponentExtractor:
    ...
    def extract_all_components(self) -> Dict[str, ComponentMetadata]:
        ...
        for component_name, mapping in component_mappings.items():
            ...
            if (
                metadata and metadata.source_files
            ):  # Only include if has actual source files
                components[component_name] = metadata
                self._generate_component_template(metadata)
                print(
                    f"✅ {component_name}: {len(metadata.source_files)} files, {len(metadata.python_dependencies)} dependencies"
                )
            else:
    ...
    def _extract_component(
        ...
        dependencies = self._extract_component_dependencies(
            source_analysis, config_analysis
        )
            ...
            for py_file in path.rglob("*.py"):
                ...
                try:
                    with open(py_file, "r", encoding="utf-8") as f:
                        ...
                        if any(
                            keyword in loaded_data.lower()
                            for keyword in ["health", "status", "ping", "ready"]
                        ):
                ...
                for f in path.rglob("*"):
                    if f.is_file():
                        ...
                        if f.suffix in [".sh", ".py"] and any(
                            ...
                            keyword in f.name.lower()
                            for keyword in ["monitor", "health", "metrics"]
                        ):
    ...
    def _extract_component_dependencies(
        ...
        dependencies["env_vars"] = list(
            set(source_analysis["env_var_usage"] + config_analysis["env_vars"])
        )
    ...
    def _templatize_file(
        self, source_path: Path, template_dir: Path, metadata: ComponentMetadata
    ):
        ...
        try:
            ...
            content = re.sub(r"mlx-platform-template", "{{PROJECT_NAME}}", content)
            content = re.sub(r"localhost:8000", "{{API_HOST}}:{{API_PORT}}", content)
    ...
    def _generate_installation_guide(
        self, component_dir: Path, metadata: ComponentMetadata
    ):
...
def main():
    ...
    print("=" * 50)
    print(
        f"✅ Extraction complete! Generated {len(components)} production-ready components:"
    )
    for name, meta in components.items():
        print(
            f"   📦 {name}: {len(meta.source_files)} files, {len(meta.python_dependencies)} dependencies"
        )

    print(f"\n📋 Registry saved to: {registry_path}")
    print("🎯 Components ready for: ./mlx add <component-name>")
